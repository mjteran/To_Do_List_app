...
def add_task(task):
    ...
    task_l = task_df['Task'].str.lower().str.strip()
    if task.lower().strip() in task_l.values:
...
def view_task():
    ...
    task_sorted = task_df.sort_values(by=['Date','Priority_Value'],ascending=[True,True])         # sort by Priority values
...
def suggest_task():
    ...
    task_sorted = task_sug.sort_values(by=['Date','Priority_Value'],ascending=[True,True])    # sort by Priority values
...
def remove_task():
    ...
    # Find rows where the 'Task' column matches the user's input
        ...
        with open(filename, 'r') as file:               # read the tasks
            for line in file:                           # read each line of the file
                ...
                if task_i.lower().strip() not in task_df['Task'].str.lower().str.strip().values:
                    ...
                    #task_df.loc[len(task_df)] = {'Task': task_i.strip(), 'Priority': priority.strip, 'Date': date.strip,
                #'Priority_Value': p_value[priority], 'Complete': status.strip()}
...
def mark_task():
    ...
    if task_df.empty:
        print("The To-do List is empty.")
        return
    task_c = input("Enter the complete task: ").strip().lower()
